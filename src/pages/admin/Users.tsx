  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button leftIcon={<UserPlus className="w-4 h-4" />} onClick={() => toast.success('Invite sent')}>Invite User</Button>
      </div>
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {filtered.map((user, i) => (
              <motion.tr key={user.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.05 }} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                ...
                <td className="px-4 py-3.5 text-sm text-slate-500">{user.institution || '—'}</td>
                <td className="px-4 py-3.5 text-xs text-slate-400">{formatDate(user.createdAt)}</td>
                <td className="px-4 py-3.5">
                  <div className="flex gap-1">
                    <Button variant="ghost" size="sm" onClick={() => toast.success('User edited')}>Edit</Button>
                    <Button variant="ghost" size="sm" className="text-rose-500 hover:text-rose-600" onClick={() => toast.error('Suspended user')}>Suspend</Button>
                  </div>
                </td>
              </motion.tr>
            ))}
